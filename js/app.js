...
function fireApp() {
  return {
    ...
    init() {
      ...
      if (!restored) {
        // Seed with sample data only when there is no saved state
        ...
        // Re-run the fit/predict so charts and results are immediately visible
        this.$nextTick(() => this.autoFitAndPredict());
      }

      // Watch primitive settings and save whenever they change
      const primitives = [
        'initialAssets', 'predictYears',
        'salaryModel', 'costModel', 'returnModel',
        'fixedReturnRate', 'salaryAlpha', 'salaryExpOffset',
      ];
    ...
    /**
     * Persist all user-facing inputs to localStorage so they survive a page reload.
     * Fitted/computed results are not stored — they are regenerated by autoFitAndPredict().
     */
    _saveState() {
      try {
        const state = {
          initialAssets:    this.initialAssets,
          predictYears:     this.predictYears,
          historicalData:   this.historicalData,
          salaryModel:      this.salaryModel,
          costModel:        this.costModel,
          returnModel:      this.returnModel,
          fixedReturnRate:  this.fixedReturnRate,
          salaryAlpha:      this.salaryAlpha,
          salaryExpOffset:  this.salaryExpOffset,
          // manual param overrides (kept so users don't lose fine-tuning)
          manualSalaryA:       this.manualSalaryA,
          manualSalaryB:       this.manualSalaryB,
          manualSalaryC:       this.manualSalaryC,
          manualSalaryK:       this.manualSalaryK,
          manualSalaryL:       this.manualSalaryL,
          manualSalaryM:       this.manualSalaryM,
          manualSaveRate:      this.manualSaveRate,
          manualInflationRate:  this.manualInflationRate,
          manualInflationSlope: this.manualInflationSlope,
          manualBellAmplitude:  this.manualBellAmplitude,
          manualBellPeakYear:   this.manualBellPeakYear,
          manualBellSigma:      this.manualBellSigma,
          manualBellBase:       this.manualBellBase,
          manualReturnRate:     this.manualReturnRate,
        };
        localStorage.setItem('fire_state', JSON.stringify(state));
      } catch (_) { /* storage unavailable — silently ignore */ }
    },

    /**
     * Load previously saved state from localStorage.
     * Returns true if state was found and applied, false if nothing was saved.
     */
    _loadState() {
      try {
        const raw = localStorage.getItem('fire_state');
        if (!raw) return false;
        const s = JSON.parse(raw);
        if (!s || !Array.isArray(s.historicalData)) return false;
        // Restore inputs
        if (s.initialAssets   != null) this.initialAssets   = s.initialAssets;
        if (s.predictYears    != null) this.predictYears    = s.predictYears;
        if (s.historicalData)          this.historicalData  = s.historicalData;
        if (s.salaryModel)             this.salaryModel     = s.salaryModel;
        if (s.costModel)               this.costModel       = s.costModel;
        if (s.returnModel)             this.returnModel     = s.returnModel;
        if (s.fixedReturnRate != null) this.fixedReturnRate = s.fixedReturnRate;
        if (s.salaryAlpha     != null) this.salaryAlpha     = s.salaryAlpha;
        if (s.salaryExpOffset != null) this.salaryExpOffset = s.salaryExpOffset;
        // Restore manual overrides
        this.manualSalaryA       = s.manualSalaryA       ?? null;
        this.manualSalaryB       = s.manualSalaryB       ?? null;
        this.manualSalaryC       = s.manualSalaryC       ?? null;
        this.manualSalaryK       = s.manualSalaryK       ?? null;
        this.manualSalaryL       = s.manualSalaryL       ?? null;
        this.manualSalaryM       = s.manualSalaryM       ?? null;
        this.manualSaveRate      = s.manualSaveRate      ?? null;
        this.manualInflationRate  = s.manualInflationRate  ?? null;
        this.manualInflationSlope = s.manualInflationSlope ?? null;
        this.manualBellAmplitude  = s.manualBellAmplitude  ?? null;
        this.manualBellPeakYear   = s.manualBellPeakYear   ?? null;
        this.manualBellSigma      = s.manualBellSigma      ?? null;
        this.manualBellBase       = s.manualBellBase       ?? null;
        this.manualReturnRate     = s.manualReturnRate     ?? null;
        return true;
      } catch (_) { return false; }
    },

    /** Wipe all saved state and reset to the built-in sample data. */
    clearSavedState() {
      try { localStorage.removeItem('fire_state'); } catch (_) { /* ignore */ }
      const cy = this.currentYear;
      this.initialAssets   = 50000;
      this.predictYears    = 30;
      this.salaryModel     = 'careerGamma';
      this.costModel       = 'fixedSaveRate';
      this.returnModel     = 'ma3';
      this.fixedReturnRate = 7;
      this.salaryAlpha     = 0.8;
      this.salaryExpOffset = 0;
      this.historicalData  = [
        { year: cy - 3, salary: 70000, costs: 50000, returnRate: 8,  assetChanges: '' },
        { year: cy - 2, salary: 75000, costs: 52000, returnRate: 12, assetChanges: '' },
        { year: cy - 1, salary: 80000, costs: 54000, returnRate: 7,  assetChanges: '' },
      ];
      this.salaryParams = null;
